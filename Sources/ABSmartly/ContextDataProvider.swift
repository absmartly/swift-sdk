...
class ContextDataProvider {
	...
	func getContextData() -> Promise<ContextData> {
		...
		let promise: Promise<ContextData> = Promise { [weak self] successCallback, errorCallback in
			...
			self.requestCompletionHandler(request, self.clientOptions.retries, self.clientOptions.timeout) {
				(contextData: ContextData?, error: Error?) in
	...
	func getContextData(_ callBack: @escaping ((_: ContextData?, _: Error?) -> Void)) {
		guard let request = requestFactory.contextRequest else {
	...
	private func requestCompletionHandler(
		_ request: URLRequest, _ retryCount: Int, _ timeout: TimeInterval,
		_ callback: @escaping ((_: ContextData?, _: Error?) -> Void)
	) {
		let session = URLSession.shared
