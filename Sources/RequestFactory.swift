...
class RequestFactory {
    ...
    var publishRequest: URLRequest? {
        ...
        request.setValue(options.version, forHTTPHeaderField: "X-Application-Version")
    ...
    private func getContextUrl(_ querryParams: [String: String]?) -> URL? {
        var urlComponents: URLComponents? = URLComponents(string: options.endpoint + ((options.endpoint.last == "/") ? "context" : "/context"))
        ...
        if let querryParams = querryParams {
            ...
            let percentEncodedQuery: String? = urlComponents?.percentEncodedQuery?.replacingOccurrences(of: "+", with: "%2B")
