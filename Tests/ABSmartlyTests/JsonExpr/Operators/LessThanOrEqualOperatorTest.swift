...
final class LessThanOrEqualOperatorTest: OperatorTest {
	let lessThanOrEqualOperator = LessThanOrEqualOperator()

	func testEvaluate() {
		XCTAssertTrue(lessThanOrEqualOperator.evaluate(evaluator, [0, 0]).boolValue)
		...
		XCTAssertFalse(lessThanOrEqualOperator.evaluate(evaluator, [1, 0]).boolValue)
		XCTAssertEqual(2, evaluator.evaluateCallsCount)
		...
		XCTAssertTrue(lessThanOrEqualOperator.evaluate(evaluator, [0, 1]).boolValue)
		XCTAssertEqual(2, evaluator.evaluateCallsCount)
		...
		XCTAssertEqual(JSON.null, lessThanOrEqualOperator.evaluate(evaluator, [JSON.null, JSON.null]))
