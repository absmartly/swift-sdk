...
final class LessThanOperatorTest: OperatorTest {
	let lessThanOperator = LessThanOperator()

	func testEvaluate() {
		XCTAssertFalse(lessThanOperator.evaluate(evaluator, [0, 0]).boolValue)
		...
		XCTAssertFalse(lessThanOperator.evaluate(evaluator, [1, 0]).boolValue)
		XCTAssertEqual(2, evaluator.evaluateCallsCount)
		...
		XCTAssertTrue(lessThanOperator.evaluate(evaluator, [0, 1]).boolValue)
		XCTAssertEqual(2, evaluator.evaluateCallsCount)
		...
		XCTAssertEqual(JSON.null, lessThanOperator.evaluate(evaluator, [JSON.null, JSON.null]))
